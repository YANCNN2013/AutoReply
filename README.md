...
## 二、环境要求
### （一）系统要求
仅支持 **Windows 系统**（脚本中通过 `os.name == "nt"` 限制，Linux 系统会直接退出）
...
## 三、前置准备
1. **准备识别用的图片文件**（需与脚本同目录）：
   - `button.png`：需要点击的按钮截图（如发送按钮、确认按钮）
   - `chinese.png`：输入法中文状态标识截图（用于判断输入法状态）
   - `copy_icon.png`：复制按钮截图（用于复制生成的回复内容）
   > 截图要求：清晰无遮挡，建议使用截图工具精确截取目标区域，避免背景干扰

2. **微信设置**：
   - 确保微信窗口在桌面可见，不要最小化
   - 调整微信窗口位置至脚本可识别范围
   - 建议将微信聊天窗口固定尺寸，避免界面变化导致识别失败

## 四、使用方法
1. 将脚本文件（`.py`）、自定义模块（`up.py`、`message.py`）和识别用图片放在同一目录
2. 打开微信，并保持至少一个聊天窗口可见
3. 运行脚本：
```bash
python auto_reply.py
```
4. 脚本启动后会有5秒倒计时，之后自动开始监控
5. 终止脚本：按下 `Ctrl + C` 即可停止运行
...
## 六、注意事项
- **屏幕分辨率**：脚本中的坐标（如 `520, 911`）是基于特定分辨率的，更换设备/分辨率后需重新调整坐标值
- **图像识别精度**：`confidence=0.9` 表示图像匹配精度为90%，识别失败时可适当调低（如0.8），但过低会导致误识别
- **权限问题**：Windows系统下需以普通用户运行，管理员权限可能导致窗口激活失败
- **防失效机制**：按钮点击时会重试2-3次，若仍失败会触发备用点击逻辑
- **安全提示**：PyAutoGUI 的 `FAILSAFE = True` 表示将鼠标移至屏幕左上角可触发紧急停止，防止脚本失控

## 七、常见问题
### Q1: 提示 ImageNotFoundException
- 检查图片文件是否存在、命名是否正确
- 重新截取目标图片，确保与当前界面完全一致
- 降低 `confidence` 参数值（如从0.9改为0.8）
